pipeline {
    ...
    environment {
        ...
        DEPLOYMENT_IMAGE = 'asia-south1-docker.pkg.dev/${GCP_PROJECT_ID}/${ARTIFACT_REGISTRY_REPOSITORY}/${IMAGE_NAME}'
        DEPLOYMENT_TAG = '${BUILD_NUMBER}'
    }
        ...
        stage('Update Kustomization and Push') {
            steps {
                script {
                    ...
                    git pull && \\
                    cd patch && \\
                    sed 's|\\${APPLICATION_NAME}|$APPLICATION_NAME|g; s|\\${DEPLOYMENT_IMAGE}|asia-south1-docker.pkg.dev/${GCP_PROJECT_ID}/${ARTIFACT_REGISTRY_REPOSITORY}/${IMAGE_NAME}|g; s|\\${DEPLOYMENT_TAG}|${BUILD_NUMBER}|g; s|\\${ENV_NAME}|$ENV_NAME|g' vars.txt > ${APPLICATION_NAME}_vars.txt && \\
